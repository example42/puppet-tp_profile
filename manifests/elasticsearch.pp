...
# which is expected to install elasticsearch package and manage its service.
# Via the resources_hash parameter is possible to pass hashes of tp::conf and
# tp::dir defines which can manage elasticsearch configuration files and
# whole dirs.
...
#   tp_profile::elasticsearch::resources:
#     tp::conf:
#       elasticsearch:
...
#     exec:
#       elasticsearch::setup:
#         command: '/usr/local/bin/elasticsearch_setup'
#         creates: '/opt/elasticsearch'
#   tp_profile::elasticsearch::options:
#     key: value
#
...
#
# @param auto_conf If to enable automatic configuration of elasticsearch based on the
#   resources_auto_conf_hash and options_auto_conf_hash parameters, if present in
#   data/common/elasticsearch.yaml. You can both override them in your Hiera files
...
#   Check tp_profile::elasticsearch::resources_auto_conf_hash in
#   data/common/elasticsearch.yaml for the auto_conf defaults.
# @param options_auto_conf_hash The default options hash if auto_conf is set.
#   Check tp_profile::elasticsearch::options_auto_conf_hash in
#   data/common/elasticsearch.yaml for the auto_conf defaults.
#
# @param resources An hash of any resource, like tp::conf, tp::dir, exec or whatever
#   to declare for elasticsearch confiuration. Can also come from a third-party
#   component modules with dedicated elasticsearch resources.
...
#   parameter.
# @param resource_lookup_method What lookup method to use for tp_profile::elasticsearch::resources
...
#  Hash               $options                 = {},
  ...
  $options=lookup('tp_profile::elasticsearch::options', Hash, $options_lookup_method, {})
  ...
  if $manage {
    ...
    # Declaration of tp_profile::elasticsearch::resources
    $resources=lookup('tp_profile::elasticsearch::resources', Hash, $resources_lookup_method, {})
