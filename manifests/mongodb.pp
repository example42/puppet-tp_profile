# tp_profile::mongodb
#
# @summary This tp profile manages mongodb with Tiny Puppet (tp)
#
# When you include this class the relevant tp::install define is declared
# which is expected to install mongodb package and manage its service.
# Via the resources_hash parameter is possible to pass hashes of tp::conf and
# tp::dir defines which can manage mongodb configuration files and
# whole dirs.
...
#   tp_profile::mongodb::ensure: present
#   tp_profile::mongodb::resources:
#     tp::conf:
#       mongodb:
#         epp: profile/mongodb/mongodb.conf.epp
#       mongodb::dot.conf:
#         epp: profile/mongodb/dot.conf.epp
#         base_dir: conf
#     exec:
#       mongodb::setup:
#         command: '/usr/local/bin/mongodb_setup'
#         creates: '/opt/mongodb'
#   tp_profile::mongodb::options:
#     key: value
#
...
#   automatically added.
#   tp_profile::mongodb::auto_conf: true
#
# @param manage If to actually manage any resource in this profile or not.
# @param ensure If to install or remove mongodb. Valid values are present, absent, latest
#   or any version string, matching the expected mongodb package version.
# @param upstream_repo If to use mongodb upstream repos as source for packages
...
# @param settings_hash An hash of tp settings to override default mongodb file
...
#
# @param auto_conf If to enable automatic configuration of mongodb based on the
#   resources_auto_conf_hash and options_auto_conf_hash parameters, if present in
#   data/common/mongodb.yaml. You can both override them in your Hiera files
...
#   Check tp_profile::mongodb::resources_auto_conf_hash in
#   data/common/mongodb.yaml for the auto_conf defaults.
# @param options_auto_conf_hash The default options hash if auto_conf is set.
#   Check tp_profile::mongodb::options_auto_conf_hash in
#   data/common/mongodb.yaml for the auto_conf defaults.
#
# @param resources An hash of any resource, like tp::conf, tp::dir, exec or whatever
#   to declare for mongodb confiuration. Can also come from a third-party
#   component modules with dedicated mongodb resources.
...
#   parameter.
# @param resource_lookup_method What lookup method to use for tp_profile::mongodb::resources
# @param resources_defaults An Hash of resources with their default params, to be merged with
#   $resources.
#
# @param auto_prereq If to automatically install eventual dependencies for mongodb.
#   Set to false if you have problems with duplicated resources, being sure that you
#   manage the prerequistes to install mongodb (other packages, repos or tp installs).
...
#  Hash               $options                 = {},
  ...
  $options=lookup('tp_profile::mongodb::options', Hash, $options_lookup_method, {})

  if $manage {
    if $no_noop {
      info('Forced no-noop mode in tp_profile::mongodb')
      noop(false)
    }
    ...
    # Declaration of tp_profile::mongodb::resources
    $resources=lookup('tp_profile::mongodb::resources', Hash, $resources_lookup_method, {})
