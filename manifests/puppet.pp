# tp_profile::puppet
#
# @summary This tp profile manages puppet with Tiny Puppet (tp)
#
# When you include this class the relevant tp::install define is declared
# which is expected to install puppet package and manage its service.
# Via the resources_hash parameter is possible to pass hashes of tp::conf and
# tp::dir defines which can manage puppet configuration files and
# whole dirs.
...
#     exec:
#       puppet::setup:
#         command: '/usr/local/bin/puppet_setup'
#         creates: '/opt/puppet'
#   tp_profile::puppet::options:
#     key: value
#
...
#   automatically added.
#   tp_profile::puppet::auto_conf: true
#
...
# @param upstream_repo If to use puppet upstream repos as source for packages
...
#
# @param auto_conf If to enable automatic configuration of puppet based on the
#   resources_auto_conf_hash and options_auto_conf_hash parameters, if present in
#   data/common/puppet.yaml. You can both override them in your Hiera files
...
#   Check tp_profile::puppet::resources_auto_conf_hash in
#   data/common/puppet.yaml for the auto_conf defaults.
# @param options_auto_conf_hash The default options hash if auto_conf is set.
#   Check tp_profile::puppet::options_auto_conf_hash in
#   data/common/puppet.yaml for the auto_conf defaults.
#
# @param resources An hash of any resource, like tp::conf, tp::dir, exec or whatever
#   to declare for puppet confiuration. Can also come from a third-party
#   component modules with dedicated puppet resources.
...
#   parameter.
# @param resource_lookup_method What lookup method to use for tp_profile::puppet::resources
...
#  Hash               $options                 = {},
  ...
  $options=lookup('tp_profile::puppet::options', Hash, $options_lookup_method, {})

  if $manage {
    if $no_noop {
      info('Forced no-noop mode in tp_profile::puppet')
      noop(false)
    }
    ...
    # Declaration of tp_profile::puppet::resources
    $resources=lookup('tp_profile::puppet::resources', Hash, $resources_lookup_method, {})
