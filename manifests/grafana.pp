# tp_profile::grafana
#
# @summary This tp profile manages grafana with Tiny Puppet (tp)
#
# When you include this class the relevant tp::install define is declared
# which is expected to install grafana package and manage its service.
# Via the resources_hash parameter is possible to pass hashes of tp::conf and
# tp::dir defines which can manage grafana configuration files and
# whole dirs.
...
#     exec:
#       grafana::setup:
#         command: '/usr/local/bin/grafana_setup'
#         creates: '/opt/grafana'
#   tp_profile::grafana::options:
#     key: value
#
...
#   automatically added.
#   tp_profile::grafana::auto_conf: true
#
...
# @param upstream_repo If to use grafana upstream repos as source for packages
...
#
# @param auto_conf If to enable automatic configuration of grafana based on the
#   resources_auto_conf_hash and options_auto_conf_hash parameters, if present in
#   data/common/grafana.yaml. You can both override them in your Hiera files
...
#   Check tp_profile::grafana::resources_auto_conf_hash in
#   data/common/grafana.yaml for the auto_conf defaults.
# @param options_auto_conf_hash The default options hash if auto_conf is set.
#   Check tp_profile::grafana::options_auto_conf_hash in
#   data/common/grafana.yaml for the auto_conf defaults.
#
# @param resources An hash of any resource, like tp::conf, tp::dir, exec or whatever
#   to declare for grafana confiuration. Can also come from a third-party
#   component modules with dedicated grafana resources.
...
#   parameter.
# @param resource_lookup_method What lookup method to use for tp_profile::grafana::resources
...
#  Hash               $options                 = {},
  ...
  $options=lookup('tp_profile::grafana::options', Hash, $options_lookup_method, {})

  if $manage {
    if $no_noop {
      info('Forced no-noop mode in tp_profile::grafana')
      noop(false)
    }
    ...
    # Declaration of tp_profile::grafana::resources
    $resources=lookup('tp_profile::grafana::resources', Hash, $resources_lookup_method, {})
