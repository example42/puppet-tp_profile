...
# which is expected to install ansible package and manage its service.
# Via the resources_hash parameter is possible to pass hashes of tp::conf and
# tp::dir defines which can manage ansible configuration files and
# whole dirs.
...
#     exec:
#       ansible::setup:
#         command: '/usr/local/bin/ansible_setup'
#         creates: '/opt/ansible'
#   tp_profile::ansible::options:
#     key: value
#
...
#
# @param auto_conf If to enable automatic configuration of ansible based on the
#   resources_auto_conf_hash and options_auto_conf_hash parameters, if present in
#   data/common/ansible.yaml. You can both override them in your Hiera files
...
#   Check tp_profile::ansible::resources_auto_conf_hash in
#   data/common/ansible.yaml for the auto_conf defaults.
# @param options_auto_conf_hash The default options hash if auto_conf is set.
#   Check tp_profile::ansible::options_auto_conf_hash in
#   data/common/ansible.yaml for the auto_conf defaults.
#
# @param resources An hash of any resource, like tp::conf, tp::dir, exec or whatever
#   to declare for ansible confiuration. Can also come from a third-party
#   component modules with dedicated ansible resources.
...
#   parameter.
# @param resource_lookup_method What lookup method to use for tp_profile::ansible::resources
...
#  Hash               $options                 = {},
  ...
  $options=lookup('tp_profile::ansible::options', Hash, $options_lookup_method, {})
  ...
  if $manage {
    ...
    # Declaration of tp_profile::ansible::resources
    $resources=lookup('tp_profile::ansible::resources', Hash, $resources_lookup_method, {})
