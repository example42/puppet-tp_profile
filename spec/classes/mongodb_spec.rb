...
describe 'tp_profile::mongodb' do
  on_supported_os(facterversion: '2.4').select { |k, _v| k == 'redhat-7-x86_64' || k == 'ubuntu-16.04-x86_64' }.each do |os, os_facts|
    context "on #{os}" do
      ...
      describe 'with default params' do
        ...
        it { is_expected.to contain_tp__install('mongodb').with(default_params) }
      end
      ...
      describe 'with ensure => absent' do
        ...
        it { is_expected.to contain_tp__install('mongodb').with(default_params.merge('ensure' => 'absent')) }
      end
    end
  end
end
